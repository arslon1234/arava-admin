...
import { useBannerStore, useMenuCategoriesStore } from "@store";
...
const Index = ({ title, id, data }: propsData) => {
  ...
  const { postDataMenuCategories, updateDataMenuCategories } = useMenuCategoriesStore();
  ...
  const handelSubmit = async (value: any) => {
    ...
    if (!id) {
      ...
      const status = await postDataMenuCategories({
        ...value,
      ...
      const status = await updateDataMenuCategories(updateData);
      if (status === 200) {
        ...
        >
          ...
          >
            <h1 className="text-center mb-2 text-[23px] font-semibold">
              {title == "post" ? "Add a menu categories" : "Edit a menu categories"}
            </h1>
            <div className="grid grid-cols-2 gap-x-4">
              ...
              <Form.Item
                ...
                style={{ width: "100%" }}
                rules={[{ required: true }]}
                initialValue={data?.parentId ? data.parentId : 0 }
              >
                <Input typeof="number" style={{ width: "100%" }} size="large" type="number" />
              </Form.Item>

              {/* Image uploud  */}
              <div className="mt-[31px]">
                <ImageUploding text="menu-categories" />
              </div>
              {data?.imageUrl && (
                <div>
                  <img
